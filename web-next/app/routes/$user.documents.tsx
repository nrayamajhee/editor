...
export default function Dashboard() {
  ...
  return (
    <MainLayout>
      <Loader scaffold={<Spinner />} promise={Promise.all([documents, user])}>
        {([documents, user]: [Doc[], User]) => (
          <main>
            <Header user={user} />
              ...
              {documents
                ...
                .map((document: Doc) => (
                  <Document
                    document={document}
                    key={document.id}
                    link={`/${user.username}/document/${document.id}`}
                  />
                ))}
              <Form action={`/${user.username}/documents`} method="POST">
